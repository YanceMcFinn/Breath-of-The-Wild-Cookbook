...
export function Card(props: { imgUrl: string, name: string, effect: string, description: string, hearts: number, gameMode: string }){
    const heartsRestored = () => {
        let _hearts = props.hearts
        if(props.hearts == 20.5){
            _hearts = 2.5
        }
    ...
    return(
        <div className="card-bg aspect-auto flex flex-col px-0 py-0 border-solid border-2 border-sheikah-slate-blue-500 text-sheikah-slate-blue-500 glow rounded-md p-2 text-center">
            ...
            <div className="px-3 grow content-around flex flex-col">
                <h5 className="font-bold w-100 hyliaFont text-2xl text-center">{props.name}</h5>
                ...
                <div className="text-left mt-auto">
                    <a href={`${props.gameMode}/materials/${props.name.replace(" ","_")}`}><button className="bg-sheikah-slate-blue-500 hover:bg-sheikah-slate-blue-300 text-white glow font-bold py-2 px-4 my-3 rounded justify-self-end">Info</button></a>
                </div>
            </div>
        </div>
    )
