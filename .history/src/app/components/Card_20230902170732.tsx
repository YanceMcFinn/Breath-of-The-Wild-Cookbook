...
export function Card(props: { imgUrl: string, name: string, effect: string, description: string }){
    return(
        <div className="card-bg aspect-auto flex flex-col !px-0 py-0 border-solid border-sheikah-slate-blue text-sheikah-slate-blue glow border-2 rounded-md inline-block p-2 text-center">
            ...
            <div className="px-3 grow content-around">
                ...
                <div className="text-left">
                    <a href={`/materials/${props.name.replace(" ","_")}`}><button className="bg-sheikah-slate-blue hover:bg-blue-700 text-white font-bold py-3 px-4 my-2 rounded justify-self-end">Info</button></a>
                </div>
            </div>
        </div>
    )
