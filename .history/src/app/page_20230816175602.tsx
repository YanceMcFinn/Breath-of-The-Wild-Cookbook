    ...
    <main>
      ...
      <div className='px-5 mb-2'>
        <div className='grid md:grid-cols-3 gap-5 px-5'>
          {materials.length != 0 ? materials?.map((item: { name: string, image: string, cooking_effect: string, description: string })=>{
            return <Card name={item.name} imgUrl={item.image} effect = {item.cooking_effect} description = {item.description} />
          }) : <div className='text-center'><h2 className='text-sheikah-slate-blue glow hyliaFont text-center'>no search results found</h2></div>}
        </div>
      </div>
      <div className='mb-2'>
