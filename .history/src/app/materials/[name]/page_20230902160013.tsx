...
export async function generateStaticParams() {
  ...
  export default async function Material ({ params }){
    ...
    return (
      <div className="text-xl card-bg m-3 w-100 h-50 grid grid-cols-2 px-0 py-0 border-solid border-sheikah-slate-blue text-sheikah-slate-blue glow border-2 block rounded-md inline-block space-y-1 space-x-1 p-2">
        ...
        <div id='right-col' className='w-70 p-2'>
          <h1 className="text-center text-2xl hyliaFont">{material.name}</h1>
          <p>{material.description}</p>
          <p><span className="hyliaFont">Cooking Effect: </span>{material.cooking_effect}</p>
        </div>
      </div>
    )
  }