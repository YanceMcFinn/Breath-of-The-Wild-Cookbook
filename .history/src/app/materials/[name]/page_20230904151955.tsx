...
export async function generateStaticParams() {
  ...
  export default async function Material ({ params }){
    ...
    return (
      <div className="card-bg m-3 w-100 h-50 grid md:grid-cols-2 px-0 py-0 border-solid border-sheikah-slate-blue-500 text-sheikah-slate-blue-500 glow border-2 block rounded-md inline-block space-y-1 space-x-1 p-2">
        <div id='left-col' className="md:w-30 w-full">
          <img className='w-full object-cover p-0 rounded-l-lg' src={material.image}></img>
        </div>
        <div id='right-col' className='md:w-70 w-full p-2'>
          <div className="text-center">
          </div>
          <h1 className="leading-10 text-center main-title hyliaFont">{material.name}</h1>
