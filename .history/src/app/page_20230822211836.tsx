...
import { loadBOTWMaterials, loadTOTKMaterials } from '../../lib/materials';

import Image from 'next/image'
import { Card } from './components/Card'
import '../../HyliaSerif_WebfontKit/stylesheet.css'
export async function getStaticProps() {
  const BOTWData = loadBOTWMaterials();
  const TOTKData = loadTOTKMaterials();
  return {
    props: {
      BOTWData,
      TOTKData
...
export default function Home({ BOTWData, TOTKData } : any) {
  ...
  function loadMaterials(){
    if(gameMode == 'botw'){
      setFullMaterialList(BOTWData)
    }
    else {
      setFullMaterialList(TOTKData)
    }
  }
  ...
  return (
    <main>
      ...
      <div className="inline-flex card-bg rounded-md shadow-sm" role="group">
        ...
        <button type="button" onClick={()=>setGameMode('totk')} className={"px-4 py-2 text-sm font-medium hyliaFont text-sheikah-slate-blue glow border-t border-b border-r border-sheikah-slate-blue rounded-r-lg" + " " + totkSelected}>
      ...
      {materials.length != 0 ? <div className='grid md:grid-cols-3 gap-5 justify-center'>
          {materials?.map((item: { name: string, image: string, cooking_effect: string, description: string, id: number })=>{
            return <Card key={item.id} name={item.name} imgUrl={gameMode == 'botw' ? item.image : '/BOTWCookBook_ImgComingSoon.png'} effect = {item.cooking_effect} description = {item.description} />
