...
export async function generateStaticParams() {
  ...
  export default async function Material ({ params }: { params: { name: string } }){
    ...
    return (
      <div className="card-bg m-4 w-100 h-50 grid md:grid-cols-2 px-0 py-0 border-solid border-sheikah-slate-blue-500 text-sheikah-slate-blue-500 glow border-2 block rounded-lg inline-block space-y-1 space-x-1 p-2">
        <div id='left-col' className="md:w-30 w-full">
          <img className='w-full object-cover p-0 rounded-l' src={material.image}></img>
        </div>
